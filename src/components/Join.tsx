    ...
    return (
        <Wrapper w={'800px'}>
            ...
            <Form onSubmit={onSubmit} w='420px'>
                <Input w='420px' onChange={onChange} name='code' value={code} placeholder='딜러코드' required/>
                <Input w='420px' onChange={onChange} name='id' value={id} placeholder='아이디 (담당자명)' required/>
                <Input w='420px' onChange={onChange} type='password' name='password' value={password} placeholder='비밀번호'
                       required/>
                <Input w='420px' onChange={onChange} type='password' name='password-check' value={passwordCheck}
                       placeholder='비밀번호 확인' required/>
                <NiceAuth>
                    <NiceInput onChange={onChange} name='phone' value={phone} placeholder='휴대폰번호' required/>
                    <NiceAuthButton onClick={onAuth}>본인인증</NiceAuthButton>
                </NiceAuth>
                <Input onChange={onChange} type='email' name='email' value={email} placeholder='이메일' required/>
                <SubmitInput type='submit' id='submit' style={{display: 'none'}}/>
            </Form>
            <Remembers>
                <Remember mb='5px'>
                    <CheckboxInput onClick={onClick} type='checkbox' id='remember1' name='remember1'/>
                    <Label htmlFor='remember1' flex='0 0 770px'>(필수) 회원 가입 시 등록 정보가 본사와 해당 딜러 관리자에게 전송되며, 승인
                        후 로그인 가능합니다</Label>
                </Remember>
                <Remember mb='20px'>
                    <CheckboxInput onClick={onClick} type='checkbox' id='remember2' name='remember2'/>
                    <Label htmlFor='remember2' flex='0 0 770px'>(필수) 개인정보 약관 동의</Label>
                </Remember>
            </Remembers>
